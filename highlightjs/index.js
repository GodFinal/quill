...
function compileLanguage(language) {
	...
	function joinRe(regexps, separator) {
		...
		//   follow the '(' with a '?'.
		let backreferenceRe = /\[(?:[^\\\]]|\\.)*\]|\(\??|\\([1-9][0-9]*)|\\./;
		let numCaptures = 0;
		let ret = '';
		for (let i = 0; i < regexps.length; i++) {
			let offset = numCaptures;
			let re = reStr(regexps[i]);
			if (i > 0) {
				ret += separator;
			}
			while (re.length > 0) {
				let match = backreferenceRe.exec(re);
				if (match == null) {
					ret += re;
					break;
				}
	...
	function compileMode(mode, parent) {
		...
		if (mode.keywords) {
			let compiled_keywords = {};

			let flatten = function (className, str) {
...
function highlight(name, value, ignore_illegals, continuation) {
	...
	function keywordMatch(mode, match) {
		let match_str = language.case_insensitive ? match[0].toLowerCase() : match[0];
	...
	function processSubLanguage() {
		let explicit = typeof top.subLanguage === 'string';
	...
	function processLexeme(buffer, lexeme) {
		...
		let new_mode = subMode(lexeme, top);
		if (new_mode) {
			if (new_mode.skip) {
				mode_buffer += lexeme;
			} else {
		...
		let end_mode = endOfMode(top, lexeme);
		if (end_mode) {
			let origin = top;
			if (origin.skip) {
				mode_buffer += lexeme;
...
function highlightAuto(text, languageSubset) {
	...
	languageSubset.filter(getLanguage).filter(autoDetection).forEach(function (name) {
		let current = highlight(name, text, false);
...
function highlightBlock(block) {
	...
	let language = blockLanguage(block);
...
function registerLanguage(name, language) {
	let lang = languages[name] = language(hljs);
